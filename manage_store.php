...
$stmt = mysqli_prepare($conn, "SELECT u.*, s.store_name, s.id AS seller_id, s.last_name_change FROM users u LEFT JOIN sellers s ON u.id=s.id WHERE u.id=?");
        ...
        if($name_locked && $store_name != $user['store_name']){
            ...
            if($store_name != $user['store_name']){
                $stmt = mysqli_prepare($conn,"
                    ...
                    WHERE id=?
                ");
                mysqli_stmt_bind_param($stmt,"ssi",$store_name,$today,$user['seller_id']);
                mysqli_stmt_execute($stmt);
            }
